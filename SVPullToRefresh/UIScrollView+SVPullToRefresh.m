...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        else if(self.scrollView.isDragging && self.state == SVPullToRefreshStateStopped && contentOffset.y < 0){
            UIEdgeInsets contentInset = self.scrollView.contentInset;
            self.scrollView.contentInset = UIEdgeInsetsMake(contentOffset.y*-1, contentInset.left, contentInset.bottom, contentInset.right);
            if (self.timer) {
                [self.timer invalidate];
            }
            self.timer = [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(setupContentInset) userInfo:nil repeats:NO];
        }
    } else {
